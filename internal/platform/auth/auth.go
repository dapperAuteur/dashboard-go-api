...
// During the transition period, tokens signed with the old and new keys can coexist by looking up the correct public key by key id (kid).
//
// * Key-id-to-public-key resolution is usually accomplished via a public JWKS endpoint.
...
// This is easy for development but in production should be replaced with a caching layer that calls a JWKS endpoint.
...
// Authenticator is used to authenticate claims.
...
func NewAuthenticator(privateKey *rsa.PrivateKey, activeKID, algorithm string, publicKeyLookupFunc KeyLookupFunc) (*Authenticator, error) {
	...
	// The algorithm used to sign the JWT must be validated to avoid a critical vulnerability:
...
// ParseClaims recreates the Claims that were used to generate a token.
// It verifies that the token was signed using our key.
func (a *Authenticator) ParseClaims(tokenStr string) (Claims, error) {
	
	// f is a function that returns the public key for validating a token.
	// We use the parsed (but unverified) token to find the key id.
	// That ID is passed to our KeyFunc to find the public key to use for verification.
